    ...
    def request(self, host, handler, request_body, verbose=0):
        ...
        if not self.mysessid is None:
            urlopener.addheaders.append(
                ("Cookie", "%s=%s" % (self.session_name, self.mysessid)))
...
class DrupalFormatRegistry():
    ...
    def __init__(self, config):
        ...
        self.server = xmlrpclib.Server(
            config['url'], allow_none=True, transport=p)
        ...
        self.session = self.server.user.login(
    ...
    def find_node_for_puid(self, puid):
        try:
            found = self.server.node.index(
                '"'+puid+'"', "true")  # , "field_puid" )
    ...
    def push_pronom(self, source):
        ...
        if(hasattr(ff, "Developers")):
            node['field_creator'] = [
                {'value': ff.Developers.DeveloperCompoundName.text.strip()}]
        ...
        if(hasattr(ff, "InternalSignature")):
             ...
             for isg in ff.InternalSignature:
                 ...
                 for bs in isg.ByteSequence:
                     ...
                     regex = fido.prepare.convert_to_regex(
                         regex, endianness, pos, offset, maxoffset)
        ...
        if(hasattr(ff, "Document")):
             ...
             for doc in ff.Document:
                 ...
                 if(hasattr(doc, 'Author')):
                     content['field_doc_author'] = [
                         {"value": doc.Author.AuthorCompoundName.text.strip()}]
                 if(hasattr(doc, 'Publisher')):
                     content['field_doc_publisher'] = [
                         {"value": doc.Publisher.PublisherCompoundName.text.strip()}]
	...
	# This works for autocomplete lists: node['field_same_as'] = {'und': [{'nid': '[nid:43]'}] }
        ...
        if(hasattr(ff, 'RelatedFormat')):
            for rf in ff.RelatedFormat:
                rf_node_id = self.find_node_for_format(
                    rf.RelatedFormatName.text.strip(), rf.RelatedFormatVersion.text.strip())
                if(rf_node_id != -1):
                    rf_node_id = rf_node_id['nid']
                    rf_node_id = rf.RelatedFormatName.text.strip(
                    )+" "+rf.RelatedFormatVersion.text.strip()+" [nid:"+str(rf_node_id)+"]"
                    print rf_node_id
                    if(rf.RelationshipType.text == "Equivalent to"):
                        node['field_same_as']['und'].append(
                            {'nid': rf_node_id})
                    if(rf.RelationshipType.text == "Has lower priority than"):
                        node['field_lower_priority_than']['und'].append(
                            {'nid': rf_node_id})
                    if(rf.RelationshipType.text == "Is subsequent version of"):
                        node['field_subsequent_version']['und'].append(
                            {'nid': rf_node_id})
                    if(rf.RelationshipType.text == "Is subtype of"):
                        node['field_conforms_to']['und'].append(
                            {'nid': rf_node_id})
...
#            node['field_conforms_to'] =  [{'nid': ''}]
        ...
        try:
            ...
            # DEBUG - get the final node - not needed now that we know it works
            nn = self.server.node.retrieve(n, {})
        ...
        #pp.pprint(nn['title'])
        #pp.pprint(n) # DEBUG
        # DEBUG - dump the final node - not needed now that we know it works
        pp.pprint(nn)
